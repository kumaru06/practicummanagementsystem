<?php 
    ...
    $grade = (float)($studentEvaluation['coordinator_grade'] ?? 0);
    ...
    $evalAudienceNote = $evalAudienceNote ?? 'This student evaluation of the coordinator is private and is not visible to coordinators or the Host Training Establishment.';
?>
...
<section class="card final-form-card eval-form-card final-readonly-card">
    ...
    <?php foreach ($criteria as $sectionName => $items): ?>
        <div class="eval-section">
            ...
            <?php foreach ($items as $key => $def): $rowIndex++; $r = (int)($coordinatorRatings[$key] ?? 0); ?>
                <div class="eval-row">
    ...
    <div class="final-readonly-field">
        ...
        <div class="final-readonly-value final-readonly-text"><?= nl2br(e($studentEvaluation['coordinator_comments'] ?? '')) ?: '<span class="muted">No comments.</span>' ?></div>
    </div>
</section>