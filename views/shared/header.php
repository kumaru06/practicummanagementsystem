...
$headerRoute = (string)($_GET['r'] ?? ($user['role'] ?? 'admin'));
?>
<!doctype html>
...
<body class="app-page role-<?= e($user['role'] ?? 'guest') ?><?= $sidebarCollapsed ? ' sidebar-collapsed' : '' ?><?= $studentNavLocked ? ' student-onboarding' : '' ?><?= $studentNavReveal ? ' student-nav-reveal' : '' ?><?= $headerRoute === 'chat' ? ' page-chat' : '' ?>" data-app-base="<?= e(app_base_path()) ?>" data-sidebar-collapsed="<?= $sidebarCollapsed ? '1' : '0' ?>" data-student-nav-reveal="<?= $studentNavReveal ? '1' : '0' ?>">
    ...
    <aside class="sidebar<?= $studentNavLocked ? ' sidebar--onboarding' : '' ?><?= $studentNavReveal ? ' sidebar--nav-reveal' : '' ?>">
        ...
        <nav class="nav">
            ...
            <?php if ($role === 'admin'):
                ...
                $adminPendingCounts = admin_pending_request_counts();
                $pendingRegistrationCount = (int)($adminPendingCounts['registration'] ?? 0);
                $pendingPasswordResetCount = (int)($adminPendingCounts['password_reset'] ?? 0);
                $pendingManageUsersTotal = (int)($adminPendingCounts['total'] ?? 0);
            ?><div class="nav-group <?= $userGroupOpen ? 'open' : '' ?>">
                <button class="nav-group-toggle" type="button">
                    ...
                    <?php if ($pendingManageUsersTotal > 0): ?>
                        <span class="nav-link-badge" data-admin-pending-total aria-label="<?= (int)$pendingManageUsersTotal ?> pending requests"><?= $pendingManageUsersTotal > 99 ? '99+' : (int)$pendingManageUsersTotal ?></span>
                    <?php endif; ?>
                    <svg class="chevron" viewBox="0 0 24 24"><path d="M7 10l5 5 5-5z"/></svg>
                </button>
                <div class="nav-group-items">
                    <a class="nav-link nav-sub <?= $currentRoute === 'admin_users' ? 'active' : '' ?>" href="index.php?r=admin_users"><svg viewBox="0 0 24 24"><path d="M12 3 2 8l10 5 8-4v6h2V8L12 3Zm-6 9v4c2 3 10 3 12 0v-4l-6 3-6-3Z"/></svg><span class="nav-link-text">Students</span></a>
                    <a class="nav-link nav-sub <?= $currentRoute === 'admin_coordinators' ? 'active' : '' ?>" href="index.php?r=admin_coordinators"><svg viewBox="0 0 24 24"><path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm-8 8c.8-4 3.8-6 8-6s7.2 2 8 6H4Z"/></svg><span class="nav-link-text">Coordinators</span></a>
                    <a class="nav-link nav-sub <?= $currentRoute === 'admin_partners' ? 'active' : '' ?>" href="index.php?r=admin_partners" title="Host Training Establishments"><svg viewBox="0 0 24 24"><path d="M3 21V7l6-4 6 4v14H3Zm14 0V9h4v12h-4Z"/></svg><span class="nav-link-text">Host Establishments</span></a>
                    <a class="nav-link nav-sub <?= $currentRoute === 'admin_registration_requests' ? 'active' : '' ?>" href="index.php?r=admin_registration_requests" title="Student Account Requests"><svg viewBox="0 0 24 24"><path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2Zm-8 2h6v2h-6V5ZM7 7h10v2H7V7Zm0 4h10v2H7v-2Zm0 4h7v2H7v-2Z"/></svg><span class="nav-link-text">Student Account Requests</span><?php if ($pendingRegistrationCount > 0): ?><span class="nav-link-badge" aria-label="<?= (int)$pendingRegistrationCount ?> pending account requests"><?= $pendingRegistrationCount > 99 ? '99+' : (int)$pendingRegistrationCount ?></span><?php endif; ?></a>
                    <a class="nav-link nav-sub <?= $currentRoute === 'admin_password_reset_requests' ? 'active' : '' ?>" href="index.php?r=admin_password_reset_requests" title="Password Reset Requests" data-admin-nav="password_reset"><svg viewBox="0 0 24 24"><path d="M12 2a5 5 0 0 1 5 5v3h1a2 2 0 0 1 2 2v8a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2v-8a2 2 0 0 1 2-2h1V7a5 5 0 0 1 5-5Zm0 2a3 3 0 0 0-3 3v3h6V7a3 3 0 0 0-3-3Zm0 9a2 2 0 1 0 0 4 2 2 0 0 0 0-4Z"/></svg><span class="nav-link-text">Password Reset Requests</span><?php if ($pendingPasswordResetCount > 0): ?><span class="nav-link-badge" data-admin-pending-password-reset aria-label="<?= (int)$pendingPasswordResetCount ?> pending password resets"><?= $pendingPasswordResetCount > 99 ? '99+' : (int)$pendingPasswordResetCount ?></span><?php endif; ?></a>
                </div>
            </div><?php endif; ?>
            <?php if ($role === 'admin'): ?><a class="nav-link <?= $currentRoute === 'admin_email_logs' ? 'active' : '' ?>" href="index.php?r=admin_email_logs"><svg viewBox="0 0 24 24"><path d="M20 4H4c-1.1 0-2 .9-2 2v12c0 1.1.9 2 2 2h16c1.1 0 2-.9 2-2V6c0-1.1-.9-2-2-2Zm0 4-8 5-8-5V6l8 5 8-5v2Z"/></svg><span>Email Logs</span></a><a class="nav-link <?= $currentRoute === 'admin_evaluations' ? 'active' : '' ?>" href="index.php?r=admin_evaluations"><svg viewBox="0 0 24 24"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17Z"/></svg><span>Evaluations</span></a><a class="nav-link <?= $currentRoute === 'admin_ojt_placement' ? 'active' : '' ?>" href="index.php?r=admin_ojt_placement"><svg viewBox="0 0 24 24"><path d="M10 2h4a2 2 0 0 1 2 2v2h3a1 1 0 0 1 1 1v11a2 2 0 0 1-2 2H6a2 2 0 0 1-2-2V7a1 1 0 0 1 1-1h3V4a2 2 0 0 1 2-2Zm2 4V4h-2v2h2Zm-1 9a3 3 0 1 0 0-6 3 3 0 0 0 0 6Z"/></svg><span>OJT Placement</span></a><a class="nav-link <?= $currentRoute === 'admin_programs' ? 'active' : '' ?>" href="index.php?r=admin_programs"><svg viewBox="0 0 24 24"><path d="M4 5h16v14H4V5Zm2 2v10h12V7H6Zm2 2h8v2H8V9Zm0 4h6v2H8v-2Z"/></svg><span>Degree Program</span></a><a class="nav-link <?= $currentRoute === 'admin_terms' ? 'active' : '' ?>" href="index.php?r=admin_terms"><svg viewBox="0 0 24 24"><path d="M7 2a1 1 0 0 1 1 1v1h8V3a1 1 0 1 1 2 0v1h1a3 3 0 0 1 3 3v11a3 3 0 0 1-3 3H5a3 3 0 0 1-3-3V7a3 3 0 0 1 3-3h1V3a1 1 0 0 1 1-1Zm13 8H4v8a1 1 0 0 0 1 1h14a1 1 0 0 0 1-1v-8ZM5 6a1 1 0 0 0-1 1v1h16V7a1 1 0 0 0-1-1H5Z"/></svg><span>Academic Term</span></a><a class="nav-link <?= in_array($currentRoute, ['admin_reports', 'admin_report'], true) ? 'active' : '' ?>" href="index.php?r=admin_reports"><svg viewBox="0 0 24 24"><path d="M5 3h9l5 5v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V4a1 1 0 0 1 1-1Zm8 1.5V8h3.5L13 4.5ZM8 13h2v5H8v-5Zm3.5-3h2v8h-2v-8ZM15 15h2v3h-2v-3Z"/></svg><span>Reports</span></a><a class="nav-link <?= $currentRoute === 'admin_recent_activities' ? 'active' : '' ?>" href="index.php?r=admin_recent_activities"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" style="fill:none"><path d="M3 12a9 9 0 1 0 9-9 9.75 9.75 0 0 0-6.74 2.74L3 8"/><path d="M3 3v5h5"/><path d="M12 7v5l4 2"/></svg><span>Recent Activities</span></a><a class="nav-link <?= $currentRoute === 'chat' ? 'active' : '' ?>" href="index.php?r=chat" data-chat-unread><svg class="nav-chat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg><span>Live Chat</span><span class="nav-link-badge" data-chat-unread-count hidden>0</span></a><?php endif; ?>
            <?php if ($role === 'coordinator'): ?><a class="nav-link <?= $currentRoute === 'coordinator_manage' ? 'active' : '' ?>" href="index.php?r=coordinator_manage"><svg viewBox="0 0 24 24"><path d="M12 12a4 4 0 1 0 0-8 4 4 0 0 0 0 8Zm-8 8c.8-4 3.8-6 8-6s7.2 2 8 6H4Z"/></svg><span>Student Assign</span></a><a class="nav-link <?= $currentRoute === 'coordinator_students' ? 'active' : '' ?>" href="index.php?r=coordinator_students"><svg viewBox="0 0 24 24"><path d="M4 6h16v2H4V6Zm0 5h16v2H4v-2Zm0 5h16v2H4v-2Z"/></svg><span>My Students</span></a><a class="nav-link <?= $currentRoute === 'coordinator_moa_mou' ? 'active' : '' ?>" href="<?= e(route_url('coordinator.moa_mou')) ?>"><svg viewBox="0 0 24 24"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8l-6-6Zm0 2.5L17.5 8H14V4.5ZM8 13h8v2H8v-2Zm0 4h8v2H8v-2Zm0-8h5v2H8V9Z"/></svg><span>MOA/MOU</span></a><a class="nav-link <?= $currentRoute === 'coordinator_evaluations' ? 'active' : '' ?>" href="index.php?r=coordinator_evaluations"><svg viewBox="0 0 24 24"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17Z"/></svg><span>Evaluations</span></a><a class="nav-link <?= $currentRoute === 'chat' ? 'active' : '' ?>" href="index.php?r=chat" data-chat-unread><svg class="nav-chat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg><span>Live Chat</span><span class="nav-link-badge" data-chat-unread-count hidden>0</span></a><?php endif; ?>
            <?php if ($role === 'student'): ?>
...
<?php
                ...
                <a class="nav-link <?= $currentRoute === 'chat' ? 'active' : '' ?>" href="index.php?r=chat" data-chat-unread><svg class="nav-chat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg><span class="nav-link-label nav-link-label--full">Live Chat</span><span class="nav-link-label nav-link-label--short" aria-hidden="true">Chat</span><span class="nav-link-badge" data-chat-unread-count hidden>0</span></a>
            ...
            <?php endif; ?>
            <?php if ($role === 'partner'): ?>
                <a class="nav-link <?= $currentRoute === 'partner_portal' ? 'active' : '' ?>" href="<?= e(route_url('partner.portal')) ?>"><svg viewBox="0 0 24 24"><path d="M3 21V7l6-4 6 4v14h-4v-5H7v5H3Zm14 0V9h4v12h-4ZM7 9h4v2H7V9Zm0 4h4v2H7v-2Z"/></svg><span>Host Training Establishment Portal</span></a>
                <a class="nav-link <?= $currentRoute === 'partner_submissions' ? 'active' : '' ?>" href="<?= e(route_url('partner.submissions')) ?>"><svg viewBox="0 0 24 24"><path d="M19 3H5a2 2 0 0 0-2 2v14a2 2 0 0 0 2 2h14a2 2 0 0 0 2-2V5a2 2 0 0 0-2-2Zm-9 14-4-4 1.4-1.4 2.6 2.6 5.6-5.6L17 10l-7 7Z"/></svg><span>Student Submissions</span></a>
                <a class="nav-link <?= $currentRoute === 'partner_timeline' ? 'active' : '' ?>" href="<?= e(route_url('partner.timeline')) ?>"><svg viewBox="0 0 24 24"><path d="M7 3a2 2 0 0 1 2 2v1h6V5a2 2 0 1 1 4 0v14a2 2 0 0 1-2 2H7a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2Zm0 5v11h10V8H7Zm2 2h6v2H9v-2Zm0 4h4v2H9v-2Z"/></svg><span>Activity Timeline</span></a>
                <a class="nav-link <?= $currentRoute === 'partner_reports' ? 'active' : '' ?>" href="<?= e(route_url('partner.reports')) ?>"><svg viewBox="0 0 24 24"><path d="M4 20V4h16v16H4Zm2-2h12V6H6v12Zm2-8h2v6H8v-6Zm4-3h2v9h-2v-9Zm4 5h2v4h-2v-4Z"/></svg><span>Reports</span></a>
                <a class="nav-link <?= $currentRoute === 'partner_evaluations' ? 'active' : '' ?>" href="<?= e(route_url('partner.evaluations')) ?>"><svg viewBox="0 0 24 24"><path d="M9 16.17 4.83 12l-1.42 1.41L9 19 21 7l-1.41-1.41L9 16.17Z"/></svg><span>Evaluations</span></a>
                <a class="nav-link <?= $currentRoute === 'chat' ? 'active' : '' ?>" href="<?= e(route_url('chat')) ?>" data-chat-unread><svg class="nav-chat-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round" aria-hidden="true"><path d="M7.9 20A9 9 0 1 0 4 16.1L2 22Z"/></svg><span>Live Chat</span><span class="nav-link-badge" data-chat-unread-count hidden>0</span></a>
                <a class="nav-link <?= in_array($currentRoute, ['partner_settings', 'partner_password', 'partner_profile'], true) ? 'active' : '' ?>" href="<?= e(route_url('partner.settings')) ?>"><svg viewBox="0 0 24 24"><path d="M19.14 12.94c.04-.31.06-.63.06-.94s-.02-.63-.06-.94l2.03-1.58a.5.5 0 0 0 .12-.64l-1.92-3.32a.5.5 0 0 0-.6-.22l-2.39.96a7.03 7.03 0 0 0-1.63-.94l-.36-2.54A.5.5 0 0 0 13.9 2h-3.8a.5.5 0 0 0-.49.42l-.36 2.54c-.58.23-1.12.54-1.63.94l-2.39-.96a.5.5 0 0 0-.6.22L2.31 8.48a.5.5 0 0 0 .12.64l2.03 1.58c-.04.31-.06.63-.06.94s.02.63.06.94l-2.03 1.58a.5.5 0 0 0-.12.64l1.92 3.32a.5.5 0 0 0 .6.22l2.39-.96c.51.4 1.05.71 1.63.94l.36 2.54a.5.5 0 0 0 .49.42h3.8a.5.5 0 0 0 .49-.42l.36-2.54c.58-.23 1.12-.54 1.63-.94l2.39.96a.5.5 0 0 0 .6-.22l1.92-3.32a.5.5 0 0 0-.12-.64l-2.03-1.58ZM12 15.5A3.5 3.5 0 1 1 12 8a3.5 3.5 0 0 1 0 7.5Z"/></svg><span>Settings</span></a>
            <?php endif; ?>
        </nav>
        ...
        <header class="topbar">
            <div class="topbar-leading">
                <div class="topbar-copy">
                    ...
                    <h1><?= e($pageTitle) ?></h1>
                    <?php if (!empty($topbarHint)): ?>
                        <p class="topbar-hint"><?= e($topbarHint) ?></p>
                    <?php endif; ?>
                </div>
            </div>
                ...
                <div class="topbar-toolbar">
                    <div class="notification-menu" id="notifMenu">
                        <button class="notif-trigger" id="notifBtn" type="button" aria-label="Notifications" aria-controls="notifPanel" aria-expanded="false">
                            ...
                            <?php if (($unreadNotifications ?? 0) > 0): ?><span class="notif-badge" data-notif-unread-count><?= (int)$unreadNotifications ?></span><?php endif; ?>
                        </button>
                    </div>
