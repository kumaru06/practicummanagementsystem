...
function initCharacterCounters() {
    document.querySelectorAll('textarea').forEach(textarea => {
        if (textarea.id === 'chatMessageInput' || textarea.closest('.chat-window__composer')) {
            return;
        }
...
function updateAppTopbar(title, hint) {
    ...
    if (copy) {
        let hintEl = copy.querySelector('.topbar-hint');
        const text = String(hint || '').trim();
        if (text) {
            if (!hintEl) {
                hintEl = document.createElement('p');
                hintEl.className = 'topbar-hint';
        ...
        const eyebrow = copy.querySelector('.topbar-eyebrow');
        if (eyebrow) {
            eyebrow.hidden = Boolean(text);
        }
    }
    document.body.classList.toggle('page-chat', Boolean(document.getElementById('chatApp')));
}
...
function initLiveChat() {
    if (typeof window.initLiveChatPage === 'function') {
        window.initLiveChatPage();
    }
    if (typeof window.initChatUnreadNav === 'function') {
