...
class Student
{
     ...
     *
     * stage 1 = pre-deployment pack (existing pipeline), stage 2 = endorsement/recommendation,
    ...
    public const REQUIREMENTS = [
        ...
        'evaluation_form' => ['name' => 'Evaluation Form', 'stage' => 3, 'owner' => 'student', 'notes' => 'Accomplished evaluation form.'],
        'coc' => ['name' => 'Certificate of Completion (COC)', 'stage' => 3, 'owner' => 'student', 'notes' => 'Certificate of Completion issued after OJT.'],
    ...
    public function canUploadRequirement(int $studentId, string $key): bool
    {
        ...
        if ($stage === 2) {
            ...
            if ($predeploymentStatus === 'orientation_completed') {
                return false;
            }
            $deploymentStatus = (string)($enrollment['status'] ?? 'pending');
            if (in_array($deploymentStatus, ['active', 'completed'], true)) {
                return false;
            }
        ...
        // Stage 3: per-file. Allow (re)upload when empty or rejected; lock once uploaded/approved.
        if ($status === 'rejected') {
            return true;
        }
        return !$hasFile;
    ...
    public function requirementUploadMessage(int $studentId, string $key): string
    {
        ...
        if ($stage === 2 && !$hasFile) {
            $predeploymentStatus = $this->normalizePredeploymentStatus($enrollment['predeployment_status'] ?? 'not_submitted');
            if ($predeploymentStatus === 'orientation_completed') {
                return 'Upload before orientation is completed';
            }
            $deploymentStatus = (string)($enrollment['status'] ?? 'pending');
            if (in_array($deploymentStatus, ['active', 'completed'], true)) {
                return 'Upload before OJT deployment begins';
            }
        }
