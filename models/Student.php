...
class Student
{
    ...
    public const REQUIREMENTS = [
        ...
        'endorsement_letter' => ['name' => 'Endorsement Letter', 'stage' => 2, 'owner' => 'coordinator', 'notes' => 'Generated automatically when your coordinator forwards your approved documents to the Host Training Establishment.'],
    ...
    public function allForAdmin(): array
    {
        ...
        $rows = $this->db->query(
            'SELECT s.*, u.id AS user_id, u.first_name, u.middle_name, u.last_name, u.name, u.email, u.is_active,
                    ...
                    e.official_start_date, e.projected_end_date, e.endorsement_file,
    ...
    public function saveRequirement(int $studentId, string $key, string $filePath): void
    {
        ...
        if ($this->requirementStage($key) === 1) {
            ...
            if ($enrollment) {
                ...
                if (($existing['status'] ?? '') === 'rejected' || $predeployment === 'needs_revision') {
     ...
     * stage 1: student record exists (no company/enrollment required).
     * stage 2: all 1st to Comply documents approved.
     * stage 3: actively deployed.
     */
    public function canAccessStage(int $studentId, int $stage): bool
    {
        ...
        if ($stage === 2) {
            return $this->hasApprovedRequirements($studentId);
        }
        if ($stage === 3) {
            $enrollment = (new Enrollment($this->db))->detailsByStudent($studentId);
            if (!$enrollment) {
    ...
    public function stageAggregateStatus(int $studentId, int $stage): string
    {
        ...
        if ($stage === 1) {
            ...
            // except when there is no enrollment yet — uploaded files are already with the coordinator.
            if ($withFile === $total) {
                $enrollment = (new Enrollment($this->db))->detailsByStudent($studentId);
                return $enrollment ? 'in_progress' : 'submitted';
            }
    ...
    public function requirementUploadMessage(int $studentId, string $key): string
    {
        ...
        if ($owner === 'coordinator') {
            if ($hasFile) {
                return 'Provided by coordinator';
            }
            if ($key === 'endorsement_letter') {
                return 'Your coordinator will generate this letter when they forward your documents to the company.';
            }
        ...
        if ($stage === 1) {
            $predeploymentStatus = $this->normalizePredeploymentStatus(
                is_array($enrollment) ? ($enrollment['predeployment_status'] ?? 'not_submitted') : 'not_submitted'
            );
            if (!$hasFile) {
                return 'Ready to upload';
            }
            if ($predeploymentStatus === 'submitted' || !$enrollment) {
                return 'Under review';
            }
    ...
    {
        $enrollment = (new Enrollment($this->db))->byStudent($studentId);
        $current = $this->normalizePredeploymentStatus(
            is_array($enrollment) ? ($enrollment['predeployment_status'] ?? 'not_submitted') : 'not_submitted'
        );
        if ($this->isPredeploymentPipelineAdvanced($current)) {
            return $current;
